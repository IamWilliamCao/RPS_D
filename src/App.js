...
import { getMapMusic, getBattleMusic, playMusic, playSFX, updateMusicVolume, updateSFXVolume} from './audioManager';
...
function App() {
  ...
  const [showDungeon, setShowDungeon] = useState(introComplete);
  const [inBattle, setInBattle] = useState(false);
  const [showRewards, setShowRewards] = useState(false);
  const [flip, setFlip] = useState(false);
  const [fadeDungeonOut, setFadeDungeonOut] = useState(false); // <=== new state
  const [currentTile, setCurrentTile] = useState(() => {
    const saved = localStorage.getItem('currentTile');
  ...
  const [mapViewDungeon, setMapViewDungeon] = useState(1);
  ...
  const [randomBlankTrait, setRandomBlankTrait] = useState(null);
  const [randomBlankIndex, setRandomBlankIndex] = useState(null);
  const [webbedCards, setWebbedCards] = useState([]);
  const [severedHandUsedOnIndexes, setSeveredHandUsedOnIndexes] = useState([]);
  ...
  function getDungeonLevel(tile) {
    if (tile <= 4) return 1;
    if (tile >= 5 && tile <= 22) return 2;
    return null;
  }
  ...
  useEffect(() => {
    if (phase === 'enemy') {
      ...
      const chosen = random[Math.floor(Math.random() * 3)];
      setRandomBlankTrait(chosen);
    }
  }, [phase]);

  useEffect(() => {
    if (playerHP <= 0 && inBattle) {
      // Add 1-second delay before showing death screen
      setTimeout(() => {
        ...
        setTimeout(() => {
          ...
          setInBattle(false);
          setShowDungeon(true);
        }, 5000); // 5s for full death screen sequence
  ...
  // MAP MUSIC
  useEffect(() => {
    if (!inBattle && mapViewDungeon !== null) {
      console.log(mapViewDungeon);
      const track = getMapMusic(mapViewDungeon);
      playMusic(track, musicVolume);
    }
  }, [inBattle, mapViewDungeon]);

  // BATTLE MUSIC
  useEffect(() => {
    if (inBattle) {
      const baseTrack = getBattleMusic(currentTile);

      // Use alternate music on every second wave (wave 1, 3, etc. => 0-based)
      const altTrack = (currentWaveIndex % 2 === 1) ? 'battle_tense' : baseTrack;

      playMusic(altTrack, musicVolume);
    }
  }, [inBattle, currentTile, currentWaveIndex]);
  ...
  useEffect(() => {
    ...
    setMapViewDungeon(Math.min(nextView, 2)); 
  }, [currentTile]);
...
function formatMoveDescription(move) {
  if (!move || !move.description) return '';

  if (move.swap === 'RandomBlank' && randomBlankTrait) {
    return move.description.replace('{trait}', randomBlankTrait);
  }

  if (move.swap === 'RandomNumBlank' && typeof randomBlankIndex === 'number') {
    const ordinal = getOrdinal(randomBlankIndex + 1); // Add 1 for 1-based index
    return move.description.replace('{num}', ordinal);
  }
...
useEffect(() => {
  ...
  const intervals = enemyImageRefs.current.map((el, index) => {
    const enemy = enemies[index];
    if (!el || enemy?.animation === 'none') return null;
  ...
  if (phase === 'highlight' && playerPlayed.includes('Scroll of Switchroo') && !switched) {
    ...
    const timer = setTimeout(() => {
      ...
      if (indexes.length >= 2) {
        ...
        const webbedIndexes = webbedCards.map(w => w.index);

        if (!webbedIndexes.includes(a) && !webbedIndexes.includes(b)) {
          ...
          console.log("Switchroo skipped due to webbed cards:", a, b);
        }
      }
    }, 1);
...
}, [phase, playerPlayed, switched, webbedCards]);
...
let currentPlayed = playerPlayed.map(card => {
  if (!card) return null;
  const types = cardData[card]?.type || [];
  return types.includes(randomBlankTrait) ? null : card;
});
...
if (phase === 'shoot') {
  ...
  const interval = setInterval(() => {
    ...
    if (i >= Math.max(playerPlayed.length, enemyPlayed.length)) {
      ...
      setBannedCardsThisBattle(prev => [...new Set([...prev, ...bannedThisRound])]); 
  ...
  if (nextWaveIndex < enemyWaves.length) {
    ...
    if (currentTile === 18) {
      setDialogVisible(true);
      setDialogName('');
      setDialogText("Thanks for playing the demo! More updates and areas will be coming in future builds.");
    }
    ...
    let p = currentPlayed[i - 1];
    const eObj = enemyPlayed[i - 1];
    const e = eObj?.type;
    const moveId = eObj?.id;
    let localPlayerPlayed = [...playerPlayed];
    ...
    if (Array.isArray(e) && e.includes('SwapEnemy')) {
      ...
      if (move?.swap === 'LR' && currentPlayed.length >= 2) {
        const leftIndex = 0;
        const rightIndex = currentPlayed.length - 1;
        [currentPlayed[leftIndex], currentPlayed[rightIndex]] =
          [currentPlayed[rightIndex], currentPlayed[leftIndex]];

        setPlayerPlayed([...currentPlayed]); 
      }

      if (typeof move?.swap === 'string' && move.swap.startsWith('LR_B_R_H')) {
        const match = move.swap.match(/^LR_B_R_H(\d+)$/);
        const healPerRock = match ? parseInt(match[1]) : 0;
        ...
        const leftCard = currentPlayed[leftIndex];
        const rightCard = currentPlayed[rightIndex];

        if (leftCard) {
          if (cardData[leftCard]?.type?.includes('Rock')) {
            healAmount += healPerRock;
          }
          currentPlayed[leftIndex] = null;
        }
      ...
      if (move?.swap === 'RandomBlank') {
        const updated = currentPlayed.map(card => {
          if (!card) return card;
          const types = cardData[card]?.type || [];
          return types.includes(randomBlankTrait) ? null : card;
        });
        setPlayerPlayed(updated);
      }
      
      if (move?.swap === 'RandomNumBlank') {
        const validIndexes = playerPlayed
          .map((_, idx) => idx)
          .filter(idx => playerPlayed[idx] !== 'BLANK');
        const max = validIndexes.length - 1;

        if (max >= 0) {
          const choice = validIndexes[Math.floor(Math.random() * (max + 1))];
          setRandomBlankIndex(choice);
        }
      }
    }
...
if ((Array.isArray(e) && e.includes('Immortal1')) || e === 'Immortal1') {
  ...
  if (isHealing && enemyMove?.heal) {
    ...
    console.log('Royal Healing targets:', targets);
...
if (Array.isArray(e) && e.includes('Summon')) {
  ...
  if (trueTargetIndex !== -1) {
    ...
    if (currentEnemyHPs[trueTargetIndex] === 0) {
      ...
      if (killedEnemy.death === 'web' && p) {
        const index = playerPlayed.findIndex(c => c === p);
        if (index !== -1) {
          setWebbedCards(prev => [...prev, { index, turnsLeft: 2 }]);
        }
      }
...
if (enemy.cooldownMeta?.lastMovePool !== newMovePoolKey) {
  ...
  for (const move of baseMoves) {
    ...
    if (move.swap === 'RandomBlank') {
      const traitOptions = ['Rock', 'Paper', 'Scissor'];
      const chosenTrait = traitOptions[Math.floor(Math.random() * traitOptions.length)];
      setRandomBlankTrait(chosenTrait);
    }
...
return (
    ...
    {showDeathScreen && (
      <div className="death-overlay">
        <div className="death-text">Your hand faltered<br />but it’s not empty.</div>
      </div>
    )}
    ...
    <div className={`App ${weirdMode ? 'weird' : ''}`}>
        ...
        {showSettings && (
          <>
            ...
            <div style={{ marginTop: '1.5rem' }}>
              <label style={{ display: 'block', marginBottom: '0.5rem' }}>🎵 Music Volume</label>
              <input
                type="range"
                min={0}
                max={1}
            ...
            <div style={{ marginTop: '1.5rem' }}>
              <label style={{ display: 'block', marginBottom: '0.5rem' }}>🔊 SFX Volume</label>
              <input
                type="range"
                min={0}
                max={1}
              ...
              {hoveredEnemyIndex === index && (
                <div className="enemy-tooltip">
                  ...
                  {enemy.death === 'web' && (
                    <div
                      className="keyword-tag"
                      onClick={() => setShowKeywordTooltip('Webbed')}
                      style={{ fontWeight: 'bold', display: 'inline-block', marginTop: '0.3rem', cursor: 'pointer' }}
                    >
                      Webbed
                    </div>
                  ...
                  >
                    {card === 'BLANK' ? '' : (!card || isDead ? '—' : (
                      ...
                      <>
                        {move?.name || card}
                        <div className="tooltip">
                        <strong>{move?.name}</strong>: {formatMoveDescription(move)}
                        {/*
                          {move?.heal && (
                            <>
                              <br />
            ...
            {(() => {
              ...
              return (
                      ...
                      if (switchrooActive) {
                        ...
                        if (hasSwitchroo && !switchrooUsed) {
                          ...
                          if (selectedSwapIndexes.length === 1) {
                            ...
                            if (webbedIndexes.includes(a) || webbedIndexes.includes(b)) {
                              setDialogText("Hey... webbed cards can't be moved.");
                              setDialogName("Righ");
                              setDialogVisible(true);
                              setSelectedSwapIndexes([]);
                              return;
                            ...
                            if (a !== b) {
                              ...
                              swappedIndexes.forEach((index) => {
                                ...
                                if (el) {
                                  ...
                                  setTimeout(() => el.classList.remove('swap-anim'), 400);
                                }
                              });
                ...
                >
                  ...
                  {webbedCards.some(w => w.index === i) && (
                    <img
                      src={`${process.env.PUBLIC_URL}/Sprites/Battle/web1.png`}
                      alt="Webbed"
                      style={{
                        position: 'absolute',
                        top: 0,
        ...
        {showKeywordTooltip === "Webbed" && (
          <>
            ...
            >
              ...
              When this enemy dies, it webs the position of the card that defeated it.
              <br />
              On your next turn, whatever card enters that slot will be webbed.
              <br />
              Webbed cards can’t be swapped.
              <br />
              <br />
              <em>(Click to close)</em>
            </div>
      ...
      <DungeonMap
        ...
        dungeonLevel={getDungeonLevel(currentTile)}
